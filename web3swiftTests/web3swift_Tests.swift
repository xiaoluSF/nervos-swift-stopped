...
class nervosswiftTests: XCTestCase {
    ...
    func testCombiningPublicKeys() {
        ...
        let pub1 = Nervos.Utils.privateToPublic(priv1, compressed: true)!
        let priv2 = Data(repeating: 0x02, count: 32)
        let pub2 = Nervos.Utils.privateToPublic(priv2, compressed: true)!
        ...
        let compinedPub = Nervos.Utils.privateToPublic(compinedPriv, compressed: true)
    ...
    func testIBANcreation() {
        ...
        let native = Nervos.Utils.Iban(iban)
        ...
        let fromAddress = Nervos.Utils.Iban(address)
