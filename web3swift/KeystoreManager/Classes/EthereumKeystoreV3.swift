...
public class EthereumKeystoreV3: AbstractKeystore {
    ...
    fileprivate func encryptDataToStorage(_ password: String, keyData: Data?, dkLen: Int=32, N: Int = 4096, R: Int = 6, P: Int = 1, aesMode: String = "aes-128-cbc") throws {
        ...
        guard let pubKey = Nervos.Utils.privateToPublic(keyData!) else {throw AbstractKeystoreError.keyDerivationError}
        guard let addr = Nervos.Utils.publicToAddress(pubKey) else {throw AbstractKeystoreError.keyDerivationError}
        self.address = addr
